...
Input
------
Users should choose to upload files (i.e. rtsSeeker result) in bed format, corresponding annotation file of the bed file (with :blue:`_anno.bed` sufffix), and snoRNA dataset ``human_hg38_snoRNABase_snoDB_rmRepeat.fa``, to predict snoRNA-(Ψ)target interaction.
...
Modified file
**********************
``-m`` option accept a three column file (with :blue:`_pseudoU.sites` suffix, generated by `5_ACA_scan.sh`) like below (genome_location snoRNA_info target_sequence)
...
Output
--------
Result with :blue:`_pseudoU_high_confidence.csv` suffix is the final target prediction result, which is a filter result of :blue:`_pseudoU.csv` (filter by ``Modified_score>=15 & PairNum>=9 & total_UnPair<=2``).
...
.. note:: All user input will be recorded in a plain text file with suffix :blue:`_ACAscan_config.txt` in psiFinder/config and help users to easily reload the previous config (by simply clicking ``CONFIG`` button).
